...
class MetaDataValidatorAcceptanceSpec extends Specification {
  ...
  "Regex rule" should {
    ...
    "succeed when @noHeader not set" in {
      validate(Path.fromString(basePath) / "regexRuleFailMetaData.csv", parse(basePath + "regexRuleSchemaWithoutNoHeaderSet.txt")) must beLike {
        case Success(_) => ok
      }
    }
  }
  ...
  "Multiple errors " should {
    "all be reported" in {
      validate(Path.fromString(basePath) / "multipleErrorsMetaData.csv", parse(basePath + "regexRuleSchemaWithNoHeaderSet.txt")) must beLike {
  ...
  "Combining two rules" should {
    "succeed when metadata valid" in {
      validate(Path.fromString(basePath) / "twoRulesPassMetaData.csv", parse(basePath + "twoRuleSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail when rules fail for all permutations" in {
      validate(Path.fromString(basePath) / "twoRulesFailMetaData.csv", parse(basePath + "twoRuleSchemaFail.txt")) must beLike {
        case Failure(errors) => errors.list mustEqual List(
  ...
  "An in rule" should {
    "succeed if the column value is in the rule's literal string" in {
      validate(Path.fromString(basePath) / "inRulePassMetaData.csv", parse(basePath + "inRuleSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if the column value is not in the rule's literal string" in {
      validate(Path.fromString(basePath) / "inRuleFailMetaData.csv", parse(basePath + "inRuleSchema.txt")) must beLike {
        case Failure(errors) => errors.list mustEqual List(
    ...
    "succeed if the column value is in the rule's cross referenced column" in {
      validate(Path.fromString(basePath) / "inRuleCrossReferencePassMetaData.csv", parse(basePath + "inRuleCrossReferenceSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if the column value is not in the rule's cross referenced column" in {
      validate(Path.fromString(basePath) / "inRuleCrossReferenceFailMetaData.csv", parse(basePath + "inRuleCrossReferenceSchema.txt")) must beLike {
  ...
  "An @optional column directive" should {
    "allow a column to have an empty value and ignore other rules" in {
      validate(Path.fromString(basePath) / "optionalPassMetaData.csv", parse(basePath + "optionalSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if a non empty value fails a rule" in {
      validate(Path.fromString(basePath) / "optionalFailMetaData.csv", parse(basePath + "optionalSchema.txt")) must beLike {
  ...
  "An @ignoreCase column directive" should {
    "pass a rule ignoring case" in {
      validate(Path.fromString(basePath) / "ignoreCasePassMetaData.csv", parse(basePath + "ignoreCaseSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }
  }
  ...
  "A fileExists rule" should {
    "ensure the file exists on the file system" in {
      validate(Path.fromString(basePath) / "fileExistsPassMetaData.csv", parse(basePath + "fileExistsSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "ensure the file exists on the file system" in {
      validate(Path.fromString(basePath) / "fileExistsCrossRefPassMetaData.csv", parse(basePath + "fileExistsCrossRefSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if the file does not exist on the file system" in {
      validate(Path.fromString(basePath) / "fileExistsPassMetaData.csv", parse(basePath + "fileExistsSchemaWithBadBasePath.txt")) must beLike {
        case Failure(errors) => errors.list mustEqual List(
  ...
  "Validate fail fast" should {
    ...
    "only report first error for invalid @TotalColumns" in {
      app.validate(Path.fromString(basePath) / "totalColumnsFailMetaData.csv", parse(basePath + "totalColumnsSchema.txt")) must beLike {
    ...
    "only report first rule fail for multiple rules on a column" in {
      app.validate(Path.fromString(basePath) / "rulesFailMetaData.csv", parse(basePath + "rulesSchema.txt")) must beLike {
    ...
    "succeed for multiple rules with valid metadata" in {
      app.validate(Path.fromString(basePath) / "twoRulesPassMetaData.csv", parse(basePath + "twoRuleSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }
  }

  "validate uk.gov.tna.dri.csv.validator.schema" should {

    "fail with duplicate column ids" in {
      parseSchema(Path.fromString(basePath) / "duplicateColumnIdsFailSchema.txt" ) must beLike {
    ...
    "succeed with unique column ids" in {
      validate(Path.fromString(basePath) / "duplicateColumnIdsMetaData.csv", parse(basePath + "duplicateColumnIdsPassSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }
  }
  ...
  "An 'or' rule" should {
    ...
    "succeed if either the lhs or rhs succeeds" in {
      validate(Path.fromString(basePath) / "orWithTwoRulesPassMetaData.csv", parse(basePath + "orWithTwoRulesSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if both the lhs or rhs are fail" in {
      validate(Path.fromString(basePath) / "orWithTwoRulesFailMetaData.csv", parse(basePath + "orWithTwoRulesSchema.txt")) must beLike {
    ...
    "succeed for 2 'or' rules with an 'and' rule" in {
      validate(Path.fromString(basePath) / "orWithFourRulesPassMetaData.csv", parse(basePath + "orWithFourRulesSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if 'or' rules pass and 'and' rule fails" in {
      validate(Path.fromString(basePath) / "orWithFourRulesFailMetaData.csv", parse(basePath + "orWithFourRulesSchema.txt")) must beLike {
  ...
  "No arg standard rules" should {
    "succeed if all the rules are valid" in {
      validate(Path.fromString(basePath) / "standardRulesPassMetaData.csv", parse(basePath + "standardRulesSchema.txt")) must beLike {
        case Success(_) => ok
      }
    }

    "fail if all the rules are not" in {
      validate(Path.fromString(basePath) / "standardRulesFailMetaData.csv", parse(basePath + "standardRulesSchema.txt")) must beLike {
