...
class MetaDataValidatorAppSpec extends Specification {
  ...
  "Check arguments" should {
    "give usage message when no arguments supplied" in {
      MetaDataValidatorCommandLineApp.checkFileArguments(Nil) must beLike { case Failure(errors) => errors.head mustEqual "Usage: validate [--failFast] <meta-data file path> <schema file path>" }
    }

    "give usage message when one argument supplied" in {
      MetaDataValidatorCommandLineApp.checkFileArguments(List("meta file")) must beLike { case Failure(errors) => errors.head mustEqual "Usage: validate [--failFast] <meta-data file path> <schema file path>"}
    }

    "give usage message when too many arguments supplied" in {
      MetaDataValidatorCommandLineApp.checkFileArguments(List("somMetaData.csv", "someSchema.txt", "something extra")) must beLike { case Failure(errors) => errors.head mustEqual "Usage: validate [--failFast] <meta-data file path> <schema file path>" }
    }
  ...
  "Fail fast check" should {
    ...
    "succeed if --failFast argument has been supplied" in {
      MetaDataValidatorCommandLineApp.failFast(List("--failFast", "someMetaData","someSchema")) must beTrue
    }

    "fail if --failFast argument has not been supplied" in {
      MetaDataValidatorCommandLineApp.failFast(List("someMetaData","someSchema")) must beFalse
    }
