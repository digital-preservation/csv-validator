...
class InRuleSpec extends Specification{
  ...
  "InRule with a string literal behaviour" should  {
    ...
    "succeed if inRule is at end of value" in {
      ...
      litRule.evaluate(0, Row(List(Cell("hello world")), 1), Schema(1, List(ColumnDefinition("column1")))) must be_==(Success(true))
    }
    ...
    "fail if inRule is not in value" in {
      val litRule = InRule(LiteralTypeProvider("my hello world"))
      litRule.evaluate(0, Row(List(Cell("hell world today")), 1), Schema(1, List(ColumnDefinition("column1")))) must beLike {
