...
class InRuleSpec extends Specification {
  ...
  "InRule with a string literal behaviour" should  {
    ...
    "succeed if inRule is embedded in value" in {
      ...
      inRule.evaluate(0, Row(List(Cell("hello world")), 1), Schema(1, List(ColumnDefinition("column1")))) mustEqual Success(true)
    }
    ...
    "succeed with @IgnoreCase" in {
      val inRule = InRule(Literal(Some("hello world")))
      inRule.evaluate(0, Row(List(Cell("hello WORLD")), 1), Schema(1, List(ColumnDefinition("column1", Nil, List(IgnoreCase()))))) mustEqual Success(true)
    }
  }
}