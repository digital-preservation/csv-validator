...
class ChecksumRuleSpec extends Specification {
  ...
  "Checksum" should  {
    ...
    "succeed when calculated algorithm does match given string value" in {
      val checksumRule = new ChecksumRule(Literal(Some("""src/test/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema/checksum.txt""")), "MD5")
    ...
    "succeed when calculated algorithm does match given string value - without /" in {
      val checksumRule = ChecksumRule(Literal(Some("""src/test/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema""")), Literal(Some("""checksum.txt""")), "MD5", emptyPathSubstitutions)
  ...
  "checksum with path substitutions" should {
    "succeed with help from substitutions to fix path" in {
      ...
      val checksumRule = new ChecksumRule(Literal(Some("""bob/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema/checksum.txt""")), "MD5", pathSubstitutions)
      checksumRule.evaluate(0, Row(List(Cell("232762380299115da6995e4c4ac22fa2")), 1), Schema(List(TotalColumns(1), NoHeader()), List(ColumnDefinition("column1")))) mustEqual Success(true)
    }
    ...
    "succeed with help from substitutions with windows path seperators" in {
      ...
      val checksumRule = new ChecksumRule(Literal(Some("bob\\resources\\uk\\gov\\tna\\dri\\uk.gov.tna.dri.csv.validator.schema\\checksum.txt")), "MD5", pathSubstitutions)
      checksumRule.evaluate(0, Row(List(Cell("232762380299115da6995e4c4ac22fa2")), 1), Schema(List(TotalColumns(1), NoHeader()), List(ColumnDefinition("column1")))) mustEqual Success(true)
    }
    ...
    "succeed when substitutions is not at the start of the path" in {
      ...
      val checksumRule = new ChecksumRule(Literal(Some("""file://bob/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema/checksum.txt""")), "MD5", pathSubstitutions)
      ...
      checksumRule.evaluate(0, Row(List(Cell("232762380299115da6995e4c4ac22fa2")), 1), Schema(List(TotalColumns(1), NoHeader()), List(ColumnDefinition("column1")))) must beLike {
        case Failure(m) => m.list mustEqual List("""checksum(file("file://bob/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema/checksum.txt"), "MD5") file "file://src/test/resources/uk/gov/tna/dri/uk.gov.tna.dri.csv.validator.schema/checksum.txt" not found for line: 1, column: column1, value: "232762380299115da6995e4c4ac22fa2"""")
      }
    }
