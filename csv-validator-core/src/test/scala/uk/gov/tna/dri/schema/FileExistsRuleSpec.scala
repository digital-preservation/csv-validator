...
class FileExistsRuleSpec extends Specification{
  ...
  "FileExistsRule" should {
    ...
    "fail for non-existent file" in {
      FileExistsRule().evaluate(0, Row(List(Cell("some/non/existent/file")), 1), Schema(globalDirsOne, List(ColumnDefinition("column1")))) must beLike {
    ...
    "fail for empty file path" in {
      FileExistsRule().evaluate(1, Row(List(Cell("abc"), Cell("")), 2), Schema(globalDirsTwo, List(ColumnDefinition("column1"), ColumnDefinition("column2")))) must beLike {
    ...
    "succeed for file that exists with no root file path" in {
      FileExistsRule().evaluate(0, Row(List(Cell("src/test/resources/uk/gov/tna/dri/schema/mustExistForRule.txt")), 1), Schema(globalDirsOne, List(ColumnDefinition("column1")))) must be_==(Success(true))
    }
