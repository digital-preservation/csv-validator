...
class FileExistsRuleSpec extends Specification{
  ...
  "FileExistsRule" should {
    ...
    "succeed for file that exists with no root file path" in {
      FileExistsRule(None).evaluate(0, Row(List(Cell("src/test/resources/uk/gov/tna/dri/schema/mustExistForRule.txt")), 1), Schema(1, List(ColumnDefinition("column1")))) must be_==(Success(true))
    }

    "succeed for file that exists with root file path" in {
      FileExistsRule(Some("src/test/resources/uk/gov/tna/")).evaluate(0, Row(List(Cell("dri/schema/mustExistForRule.txt")), 1), Schema(1, List(ColumnDefinition("column1")))) must be_==(Success(true))
    }
