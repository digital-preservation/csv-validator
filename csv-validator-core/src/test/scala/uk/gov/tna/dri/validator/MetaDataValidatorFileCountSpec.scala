...
class MetaDataValidatorFileCountSpec extends Specification {
  ...
  "FileCount with path/filename in schema" should {
    ...
    "fail when file does not exist" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema/WRONG.txt")) file "src/test/resources/uk/gov/tna/dri/schema/WRONG.txt" not found for line: 1, column: Count, value: "1""""))
      }
    }
    ...
    "fail when wrong number of files found" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema/checksum.txt")) found 1 file(s) for line: 1, column: Count, value: "2""""))
      }
    }
  ...
  "FileCount with rootpath and filename in schema" should {
    ...
    "fail when files found does match given cross referenced value" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema", "checksum.txt")) found 1 file(s) for line: 1, column: Count, value: "99""""))
      }
    }
  }
