...
class MetaDataValidatorFileCountSpec extends Specification {
  ...
  "FileCount with path/filename in schema" should {
    "succeed when it find the one file" in {
      val schema =
        """version 1.0
           ...
           File:
           Count: fileCount(file("src/test/resources/uk/gov/tna/dri/schema/checksum.txt"))
        """

      val metaData = """src/test/resources/uk/gov/tna/dri/schema/checksum.txt,1"""
    ...
    "succeed if column count is optional and missing" in {
      val schema =
        """version 1.0
           @totalColumns 2 @noHeader
           File:
           Count: fileCount(file("src/test/resources/uk/gov/tna/dri/schema/checksum.txt")) @optional
        """

      val metaData = """src/test/resources/uk/gov/tna/dri/schema/checksum.txt,"""
    ...
    "fail when file does not exist" in {
      val schema =
        """version 1.0
           @totalColumns 2 @noHeader
           File:
           Count: fileCount(file("src/test/resources/uk/gov/tna/dri/schema/WRONG.txt"))
        """

      val metaData = """src/test/resources/uk/gov/tna/dri/schema/checksum.txt,1"""

      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema/WRONG.txt")) file "src/test/resources/uk/gov/tna/dri/schema/WRONG.txt" not found for line: 1, column: Count, value: 1""")
      }
    }
    ...
    "fail when wrong number of files found" in {
      ...
      val metaData = """src/test/resources/uk/gov/tna/dri/schema/checksum.txt,2"""

      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema/checksum.txt")) found 1 file(s) for line: 1, column: Count, value: 2""")
      }
    }
  ...
  "FileCount with rootpath and filename in schema" should {
    ...
    "fail when files found does match given cross referenced value" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List("""fileCount(file("src/test/resources/uk/gov/tna/dri/schema", "checksum.txt")) found 1 file(s) for line: 1, column: Count, value: 99""")
      }
    }
  }
  ...
  "FileCount with fullpath in metedata" should {
    ...
    "fail when files found for a crossed reference does not match given value" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List("""fileCount(file($File)) 'rubbish' is not a number for line: 1, column: Count, value: rubbish""")
      }
    }
  }
  ...
  "FileCount with multi files matches" should {
    ...
    "succeed when only 1 file is found using a '*' wildcard" in {
      ...
      val metaData = """src/test/resources/uk/gov/tna/dri/schema,checksum.*,1"""
    ...
    "fail when no files found" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List("""fileCount(file("src/test/resources/this/is/incorrect", $File)) incorrect root src/test/resources/this/is/incorrect/ found for line: 1, column: Count, value: 2""")
      }
    }
  }
}