...
class MetaDataValidatorChecksumSpec extends Specification {
  ...
  "Checksum with path/filename in schema" should {
    ...
    "fail when calculated algorithm does match not given string value" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""checksum(file("src/test/resources/uk/gov/tna/dri/schema/checksum.txt"), "MD5") file "src/test/resources/uk/gov/tna/dri/schema/checksum.txt" checksum match fails for line: 1, column: MD5, value: "wrong""""))
      }
    }
  }
  ...
  "Checksum with fullpath in metedata" should {
    ...
    "fail when calculated algorithm does not match given cross referenced string value" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""checksum(file($File), "MD5") file "src/test/resources/uk/gov/tna/dri/schema/checksum.txt" checksum match fails for line: 1, column: MD5, value: "rubbish""""))
      }
    }
  }
    ...
    "fail when calculated algorithm does match" in {
      ...
      validate(metaData, schema) must beLike {
        case Failure(messages) => messages.list mustEqual List(ErrorMessage("""checksum(file("src/test/resources/uk/gov/tna/dri/schema", $File), "MD5") file "src/test/resources/uk/gov/tna/dri/schema/checksum.txt" checksum match fails for line: 1, column: MD5, value: "rubbish""""))
      }
    }
  }
