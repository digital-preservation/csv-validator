...
class CsvValidatorCmdAppSpec extends Specification with TestResources {
  ...
  val noContentSchemaPath = relResourcePath("noContentSchema.csvs")
  ...
  "Command line app" should {
    ...
    "have exit code 1 when integrity check fails because it can't find the content folder" in {
      CsvValidatorCmdApp.run(Array(metadataPath, noContentSchemaPath))._2  === 3
    }



    "have exit code 0 when validation successful with integrity check - for schema with no header" in {
      CsvValidatorCmdApp.run(Array(integrityCheckMetaDataPathNoHeader, integrityCheckSchemaPathNoHeader)) mustEqual Tuple2("PASS", 0)
    }

    "have exit code 0 when the metadata file list has more files than the actual content file" in {
      val result = CsvValidatorCmdApp.run(Array(integrityCheckMetaDataPathWithTooManyFiles, integrityCheckSchemaPath))
    ...
    "have exit code 3 when the metadata file list fewer wile than the actual content file with integrity check error message" in {
      val result = CsvValidatorCmdApp.run(Array(integrityCheckMetaDataPathMissingFiles, integrityCheckSchemaPath))
    ...
    "have exit code 0 when validation successful with integrity check" in {
       CsvValidatorCmdApp.run(Array(integrityCheckMetaDataPath, integrityCheckSchemaPath)) mustEqual Tuple2("PASS", 0)
    }
    ...
    "have exit code 0 when validation successful with integrity check on WO_95" in {
      ...
      CsvValidatorCmdApp.run(Array(s"-p:file:///WO_95=$base", integrityCheckWO95MetaDataPath, integrityCheckWO95SchemaPath)) mustEqual Tuple2("PASS", 0)
    }
